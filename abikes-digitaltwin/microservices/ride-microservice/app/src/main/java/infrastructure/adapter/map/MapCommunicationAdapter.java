...
public class MapCommunicationAdapter implements MapCommunicationPort {
    ...
    public void notifyStartRide(String bikeId, String userId) {
        ...
        logger.info("Sending start ride notification to Kafka topic: {} for user: {} and bike: {}",
        ...
        producer.send(
                ...
                (metadata, exception) -> {
                    if (exception == null) {
                        logger.info("Start ride notification sent successfully to topic: {}, partition: {}, offset: {}",
    ...
    public void notifyEndRide(String bikeId, String userId) {
        ...
        String topicName = Topics.RIDE_MAP_UPDATE.getTopicName();
        logger.info("Sending end ride notification to Kafka topic: {} for user: {} and bike: {}",
                topicName, userId, bikeId);
        ...
        producer.send(
                ...
                (metadata, exception) -> {
                    if (exception == null) {
                        logger.info("End ride notification sent successfully to topic: {}, partition: {}, offset: {}",
                                metadata.topic(), metadata.partition(), metadata.offset());
                    } else {
