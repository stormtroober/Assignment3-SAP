...
public class BikeMapServiceAPIImpl implements BikeMapServiceAPI {
            ...
            v -> {
              ...
              aBikeRepository
                  ...
                  .thenAccept(
                      bikes -> {
                        System.out.println(
                            "Publishing full ABike fleet update with " + bikes.size() + " bikes");
              ...
              aBikeRepository
                  ...
                  .thenAccept(
                      usersWithBikeMap -> {
                        System.out.println(
                            "Retrieved users with assigned bikes map, size: "
                                + usersWithBikeMap.size());
                        if (!usersWithBikeMap.isEmpty()) {
                          usersWithBikeMap.forEach(
                              (user, userBikes) -> {
                                System.out.println(
                                    "Publishing ABikes update for user "
                                        + user
                                        + " with "
                                        + userBikes.size()
                                        + " bikes");
                                  ...
                                  user ->
                                      aBikeRepository
                                          ...
                                          .thenAccept(
                                              avail -> {
                                                System.out.println(
                                                    "Publishing available ABikes to user "
                                                        + user
                                                        + " with "
                                                        + avail.size()
                                                        + " bikes");
                          ...
                          System.out.println(
                              "No users with assigned bikes, broadcasting available bikes to all");
                          aBikeRepository
                              .getAvailableBikes()
                              .thenAccept(
                                  available -> {
                  ...
                  .thenAccept(
                      publicBikes -> {
                        if (!publicBikes.isEmpty()) {
                          System.out.println(
                              "Publishing public ABikes update with "
                                  + publicBikes.size()
                                  + " bikes");
  ...
  public CompletableFuture<Void> notifyStartPublicRide(String bikeName, BikeType bikeType) {
    if (bikeType == BikeType.AUTONOMOUS) {
      return aBikeRepository.getBike(bikeName).thenCompose(aBikeRepository::assignBikeToPublic);
    } else {
      return CompletableFuture.failedFuture(
          new IllegalArgumentException("Unsupported bike type: " + bikeType));
    }
  ...
  public CompletableFuture<Void> notifyStopPublicRide(String bikeName, BikeType bikeType) {
    if (bikeType == BikeType.AUTONOMOUS) {
      return aBikeRepository.getBike(bikeName).thenCompose(aBikeRepository::unassignBikeFromPublic);
    } else {
      return CompletableFuture.failedFuture(
          new IllegalArgumentException("Unsupported bike type: " + bikeType));
    }
