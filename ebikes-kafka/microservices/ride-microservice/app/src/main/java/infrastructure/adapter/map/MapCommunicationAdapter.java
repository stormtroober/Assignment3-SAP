...
public class MapCommunicationAdapter extends AbstractVerticle implements MapCommunicationPort {
  ...
  public MapCommunicationAdapter(Vertx vertx) {
    ...
    this.mapServiceUrl =
        "http://" + mapConfig.getString("name") + ":" + mapConfig.getInteger("port");
    this.vertx = vertx;
  }
  ...
  public void notifyStartRide(String bikeId, String userId) {
    JsonObject request = new JsonObject().put("username", userId).put("bikeName", bikeId);

    webClient
        .postAbs(mapServiceUrl + "/notifyStartRide")
        .sendJsonObject(
            request,
            ar -> {
              if (ar.succeeded()) {
                ...
                System.err.println(
                    "Failed to send start ride notification: " + ar.cause().getMessage());
              }
            });
  }
  ...
  public void notifyEndRide(String bikeId, String userId) {
    ...
    webClient
        .postAbs(mapServiceUrl + "/notifyStopRide")
        .sendJsonObject(
            request,
            ar -> {
              if (ar.succeeded()) {
                ...
                System.err.println(
                    "Failed to send end ride notification: " + ar.cause().getMessage());
              }
            });
  }
}
