...
public class MongoEventStore implements EventStore {
  ...
  public CompletableFuture<Void> appendEvent(
      String aggregateId, UserEvent event, long expectedVersion) {
    ...
    switch (event.getType()) {
      case USER_CREATED:
        ...
        payload.put("userType", uc.getUserType()).put("initialCredit", uc.getInitialCredit());
        break;
  ...
  @Override
  public CompletableFuture<List<UserEvent>> loadEvents(String aggregateId, long fromSequence) {
            ...
            results -> {
              ...
              for (JsonObject doc : results) {
                ...
                switch (type) {
                  ...
                  case CREDIT_RECHARGED:
                    history.add(new CreditRecharged(aggregateId, seq, p.getInteger("amount")));
                    break;

                  case CREDIT_UPDATED:
                    history.add(
                        new domain.events.CreditUpdated(
                            aggregateId, seq, p.getInteger("newCredit")));
                    break;

                  default:
                    throw new IllegalStateException("Unknown event type in store: " + type);
                }
              }
    ...
    mongo
        ...
        .onSuccess(
            results -> {
              ...
              results.sort(
                  Comparator.comparing((JsonObject d) -> d.getString("aggregateId"))
              ...
              for (JsonObject doc : results) {
                ...
                switch (type) {
                  ...
                  case CREDIT_RECHARGED:
                    allEvents.add(new CreditRecharged(aggregateId, seq, p.getInteger("amount")));
                    break;

                  case CREDIT_UPDATED:
                    allEvents.add(new CreditUpdated(aggregateId, seq, p.getInteger("newCredit")));
                    break;
